...
import { CheckCircle2, Mic, MicOff, PhoneOff, Volume2 } from "lucide-react";
...
export default function ConversationSessionPage() {
  ...
  // Step 5 — Reusable transcript refresh function
  ...
  // Step 6 — Polling while voice is active
  React.useEffect(() => {
  ...
  // Step 9 — Clean up automatically if user leaves the page
  React.useEffect(() => {
    return () => {
      microphoneTrackRef.current?.stop();
      microphoneTrackRef.current = null;
  ...
  // Step 10 — Stop voice before ending the conversation
    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-4">
      ...
      <div className="flex flex-col gap-4 rounded-2xl border border-border bg-surface-elevated p-6 shadow-sm">
        ...
        <div className="flex items-center gap-2 border-t border-border pt-4">
          ...
          {/* Step 11 — Microphone Button */}
          {isVoiceActive ? (
            <Button size="md" variant="outline" onClick={handleStopVoice}>
        ...
        {/* Step 12 — Screen reader accessible voice status */}
        {voiceStatus ? (
          <p
            role="status"
            aria-live="polite"
